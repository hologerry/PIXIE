...
def main(args):
    ...
    for i, batch in enumerate(tqdm(testdata, dynamic_ncols=True)):
        ...
        if args.saveVis:
            ...
            cv2.imwrite(
                os.path.join(savefolder, f"{name}_vis.jpg"), visualizer.visualize_grid(visdict, size=args.render_size)
            )
        if args.saveObj:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        "--savefolder",
        default="TestSamples/hand/results",
        type=str,
    ...
    # rendering option
    parser.add_argument("--render_size", default=224, type=int, help="image size of renderings")
    parser.add_argument(
        "--rasterizer_type", default="standard", type=str, help="rasterizer type: pytorch3d or standard rasterizer"
    )
    # save
    parser.add_argument(
        "--useTex",
    ...
    parser.add_argument("--uvtex_type", default="SMPLX", type=str, help="texture type to save, can be SMPLX or FLAME")
    parser.add_argument(
        "--saveVis",
        default=True,
        type=lambda x: x.lower() in ["true", "1"],
