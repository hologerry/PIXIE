...
class Visualizer(object):
    ...
    def _setup_renderer(self):
        ...
        self.render = SRenderY(
            self.render_size, obj_filename=topology_path, uv_size=uv_size, rasterizer_type=self.rasterizer_type
        ).to(self.device)
        mask = imread(self.cfg.model.face_eye_mask_path).astype(np.float32) / 255.0
        mask = torch.from_numpy(mask[:, :, 0])[None, None, :, :].contiguous()
        self.flame_face_eye_mask = F.interpolate(mask, [self.cfg.model.uv_size, self.cfg.model.uv_size]).to(
            self.device
        )
        ...
        self.part_idx = {"head": head_ind, "left_hand": hand_ind["left_hand"], "right_hand": hand_ind["right_hand"]}
        self.color_dict = {
            "head": [180 / 255.0, 0, 0],
    ...
    def tex_flame2smplx(self, flame_texture, smplx_texture=None):
        ...
        if torch.is_tensor(flame_texture):
            ...
            flame_texture = flame_texture[0].detach().cpu().numpy().transpose(1, 2, 0)
    ...
    def light_albedo(self, opdict):
        ...
        coarse_normals = util.vertex_normals(
            opdict["vertices"], self.render.faces.expand(opdict["vertices"].shape[0], -1, -1)
        )
        uv_coarse_normals = self.render.world2uv(coarse_normals).detach()
    ...
    def render_results(
        ...
        shape_images, normal_images, grid = self.render.render_shape(
            opdict["vertices"], opdict["transformed_vertices"], background=bg, return_grid=True, h=h, w=w
        )
        ...
        if moderator_weight is not None:
            ...
            color_shape_images = self.render.render_shape(
                opdict["vertices"], opdict["transformed_vertices"], background=bg, colors=face_colors, h=h, w=w
            )
        ...
        render_ops = self.render(
            opdict["vertices"], opdict["transformed_vertices"], uvmap, background=bg, lights=lights, h=h, w=w
        )
