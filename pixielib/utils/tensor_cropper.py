...
def crop_tensor(image, center, bbox_size, crop_size, interpolation="bilinear", align_corners=False):
    ...
    if kornia.__version__ <= "0.4.0":
        cropped_image = warp_affine(
            image, dst_trans_src[:, :2, :], (crop_size, crop_size), flags=interpolation, align_corners=align_corners
        )
    else:
        cropped_image = warp_affine(
            image, dst_trans_src[:, :2, :], (crop_size, crop_size), mode=interpolation, align_corners=align_corners
        )
...
class Cropper(object):
    ...
    def transform_points(self, points, tform, points_scale=None, normalize=True):
        ...
        trans_points_2d = torch.bmm(
            torch.cat(
                [points_2d, torch.ones([batch_size, n_points, 1], device=points.device, dtype=points.dtype)], dim=-1
            ),
            tform,
        )
