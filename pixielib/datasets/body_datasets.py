...
def build_dataloader(testpath, batch_size=1):
    ...
    dataloader = DataLoader(
        dataset, batch_size=batch_size, shuffle=False, num_workers=1, pin_memory=True, drop_last=False
    )
...
class TestData(Dataset):
    def __init__(
        self, testpath, iscrop=False, crop_size=224, hd_size=1024, scale=1.1, body_detector="rcnn", device="cpu"
    ):
        """
        testpath: folder, imagepath_list, image path, video path
        """
        if os.path.isdir(testpath):
            self.imagepath_list = sorted(glob(testpath + "/*.jpg") + glob(testpath + "/*.png") + glob(testpath + "/*.jpeg"))
