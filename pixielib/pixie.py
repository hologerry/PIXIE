...
class PIXIE(object):
    ...
    def _setup_cropper(self):
        ...
        for crop_part in ["head", "hand"]:
            ...
            self.Cropper[crop_part] = tensor_cropper.Cropper(
                crop_size=data_cfg.image_size, scale=[scale_size, scale_size], trans_scale=0
            )
    ...
    def _create_model(self):
        ...
        for key in self.cfg.network.moderator.keys():
            ...
            self.Moderator[key] = TempSoftmaxFusion(
                detach_inputs=detach_inputs, detach_feature=detach_feature, channels=channels
            ).to(self.device)
        ...
        for key in data.keys():
            ...
            if key == "head" or key == "hand":
                ...
                part_dict = self.decompose_code(
                    self.Regressor[part](feature[key][part]), self.param_list_dict[f"{part}_list"]
                )
            ...
            if key == "body":
                ...
                if (
                    "head_image" not in data[key].keys()
                    ...
                    body_dict = self.decompose_code(
                        self.Regressor[part](feature[key][part]), self.param_list_dict[part + "_list"]
                    )
                    # head share
                ...
                for part_name in ["head", "left_hand", "right_hand"]:
                    ...
                    part_share_dict = self.decompose_code(
                        self.Regressor[f"{part}_share"](f_part), self.param_list_dict[f"{part}_share_list"]
                    )
    ...
    def decode(self, param_dict, param_type):
        ...
        if "tex" in param_dict.keys() and self.cfg.model.use_tex:
            ...
            prediction["albedo"] = torch.zeros(
                [verts.shape[0], 3, self.cfg.model.uv_size, self.cfg.model.uv_size],
                device=self.device,
                dtype=torch.float32,
            )
    ...
    def decode_Tpose(self, param_dict):
        ...
        verts, _, _ = self.smplx(
            shape_params=param_dict["shape"], expression_params=param_dict["exp"], jaw_pose=param_dict["jaw_pose"]
        )
        return verts
